...
class ServiceCollection
  ...
  #       ],
  def minimize_service_collection(commongetparams:, commonpostparams:) # rubocop:disable Metrics/CyclomaticComplexity,Metrics/PerceivedComplexity
    collection = {}
    commonget = {}
    commonpost = {}
    ...
    allservices.each do |s| # Service Object!
      ...
      s.paths.each_key do |fullpath|
        paths[fullpath] = {}
        paths[fullpath]['get'] = ('found' if s.paths[fullpath][:get])
    ...
    commonpostparams.each do |paramname, _param|
...
class Service
  ...
  def get_json_request(request_body:)
    ...
    # ["application/json",
    # Openapi3Parser::Node::MediaType(#/paths/~1individuals/post/requestBody/content/application~1json)]
